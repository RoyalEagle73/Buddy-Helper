...
function Open() {
    // Printing errors or calling function if all things go good
    if (document.getElementById('sourceURL').value.length == 0) {
        document.getElementById('error-box').style.display = "inline";
        document.getElementById('error-text').innerHTML = "Please enter any URL";
    } else if (!document.getElementById('sourceURL').value.startsWith("https://www.prepbytes.com/panel/mycourses/")) {
        document.getElementById('error-box').style.display = "inline";
        document.getElementById('error-text').innerHTML = "Please enter valid URL";
    } else {
        document.getElementById('error-box').style.display = "none";
        window.open(getURL(document.getElementById('sourceURL').value), "_blank ");
    }
}